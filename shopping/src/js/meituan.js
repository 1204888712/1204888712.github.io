...
var dataList;
function getData() {
	var url = "/shopping/api/list.json";
	$.ajax({
		type: "GET",
		dataType: "json",
		url: url,
	...
	// fetch(_url).then(res=>res.json()).then(res=>console.log(res)).catch(err => console.log(err))
}
// http://localhost:8080
function addList(data) {
	...
	data.list.forEach(function (ele, index) {
		str += '<li class="foodspic">\
				<a href = "/shopping/meituan-detail.html?id='+ ele.id + '" class="clearfix" >\
